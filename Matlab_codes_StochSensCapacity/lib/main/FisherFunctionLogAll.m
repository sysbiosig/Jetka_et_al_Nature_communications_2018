function [FisherTS, FisherTP, FisherDT ]= FisherFunctionLogAll(name,N,freq, init_T,y0,obs,merr)
% function calculates FIM for TS, TP and DT  data for LOG parameters
...
%log derivative
traj_derivative_val(:,:,i)=par(i)*deval(traj_derivative(i),x); %traj_derivative_val(:,:,i) contains derivative of Y with respect to ith parameter
end


%% Exctract covariance matrix at each observation time point 
...
% concatenation of calculated derivatives into a format convenient to
% evaluate FIM
