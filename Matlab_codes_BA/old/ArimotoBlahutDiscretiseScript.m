...
name='JAKSTAT7';
version_name=75;
timeNow=datestr(now,'yymmdd_HHMMSS');

dirData = ['input/',name,'/s_',num2str(version_name),'/']; 
dirresults = ['output/',name,'/s_',num2str(version_name),'_',timeNow,'/']; 

mkdir(dirresults)

allfiles=dir(dirData);
allfolders={allfiles(cellfun(@(x) (~isempty(x)),regexp({allfiles.name},'[txt]','match') )).name};

delimeter = '\t';
for dirinputname = allfolders
    
    dirinput = [dirData,dirinputname{1}];
    input_name=regexp(dirinputname{1},'^[a-zA-Z0-9_]+','match');
    diroutputglobal = [dirresults, input_name{1}, '/Output'];

    K = 0.6:0.05:0.95;
    binsnumbers = 50:100:650;
    ...
    for j = jlist
        ...
        print([diroutput,  'results/',  'all-binnumbers-sample',], '-dpdf');
        hold off;
        close(f); 
...
%%
        ...
        for bin = binsnumbers 
            ...
            if ~NoResults
                ...
                print([diroutput, 'results/', num2str(bin)], '-dpdf');
                hold off;    
                close(f); 
            end
        end
